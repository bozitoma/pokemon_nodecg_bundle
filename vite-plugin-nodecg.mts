...
export default async ({
  ...
  const generateHtmlFiles = async () => {
    const [graphicsTemplateHtml, dashboardTemplateHtml] = await Promise.all([
      fs.readFile(
        path.join(config.root, typeof template === 'string' ? template : template.graphics),
        'utf-8'
      ),
      fs.readFile(
        path.join(config.root, typeof template === 'string' ? template : template.dashboard),
        'utf-8'
      ),
    ]);
    ...
    const manifest =
      config.command === 'build'
        ? (JSON.parse(
            await fs.readFile(path.join(config.build.outDir, 'manifest.json'), 'utf-8')
          ) as Manifest)
        : undefined;

    const generateHtml = async (input: string, templateHtml: string, outputDir: string) => {
      const head: string[] = [];

      if (config.command === 'serve') {
        head.push(`
					<script type="module">
						import RefreshRuntime from '${new URL(path.join(config.base, '@react-refresh'), origin)}'
        ...
        head.push(
          `<script type="module" src="${new URL(path.join(config.base, input), origin)}"></script>`
        );
      }
      ...
      if (config.command === 'build') {
        ...
        const checkCss = (chunk: ManifestChunk) => {
          if (chunk.css) {
            for (const css of chunk.css) {
              head.push(`<link rel="stylesheet" href="${path.join(config.base, css)}">`);
            }
          }
          if (chunk.imports) {
        ...
        if (entryChunk?.file) {
          head.push(
            `<script type="module" src="${path.join(config.base, entryChunk.file)}"></script>`
          );
        }
      }
    ...
    await Promise.all([
      ...graphicsInputs.map((input) => generateHtml(input, graphicsTemplateHtml, graphicsOutdir)),
